...
export function Schedule({ kris, runs, setRunReq, kriById, setView, setFocusExc }) {
  ...
  return (
    <section>
      ...
      <h3>Run history</h3>
      <table className="tbl">
        <thead><tr><th>Run</th><th>KRI</th><th>Period</th><th>Trigger</th><th>Started</th><th>Population</th><th>Exceptions</th><th /></tr></thead>
        <tbody>
          {runs.map((r) => { const k = kriById(r.kriId); return (
            <tr key={r.id}>
              ...
              <td className="row-actions">{r.exceptions > 0 && <Btn kind="link" onClick={() => { setFocusExc(null); setView('exceptions'); }}>Open</Btn>}</td>
            </tr>
          ); })}
        </tbody>
      </table>
...
export function RunModal({ kri, onClose, onDone }) {
  const [opts, setOpts] = useState({ period: periodLabelFor(kri), region: 'All regions', bg: 'BG-A + BG-B', sampling: kri.sampling });
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-label="Run KRI now">
        ...
        {!running ? (
          <div>
            ...
            <p className="muted small">This run uses the KRI's saved steps and thresholds. Change those in the KRI library, not here.</p>
          ...
          <ol className="progress">
            {STAGES.map(([label, src], i) => (
              <li key={i} className={i < stage ? 'done' : i === stage ? 'now' : ''}><span className="dot" />{label}{src && i <= stage && <span className="muted small"> · {srcName(src)}</span>}</li>
            ))}
          </ol>
        )}
      </div>
